...
def main(args):
    ...
    if args.with_tracking:
        wandb.init(project='diffusion_watermark_alt3', tags=['tree_ring_watermark'])
        wandb.config.update(args)
        table = wandb.Table(columns=['gen_no_w', 'no_w_clip_score', 'gen_w', 'w_clip_score', 'prompt', 'no_w_metric', 'w_metric', 
                                     'fft_rev_no_w', 'fft_rev_no_w_alt', 'fft_rev_w', 'fft_rev_w_alt'])
    ...
    alt_text_embeddings = altpipe.get_text_embedding(tester_prompt)
    ...
    for i in tqdm(range(args.start, args.end)):
        ...
        fft_init_latents_w = torch.fft.fftshift(torch.fft.fft2(init_latents_w), dim=(-1, -2))[0][args.w_channel].real.detach().cpu().numpy()
        ...
        fft_reversed_latent_no_w = torch.fft.fftshift(torch.fft.fft2(reversed_latents_no_w), dim=(-1, -2))[0][args.w_channel].real.detach().cpu().numpy()

        image_latents_no_w_alt = altpipe.get_image_latents(img_no_w, sample=False)

        reversed_latents_no_w_alt = altpipe.forward_diffusion(
            latents=image_latents_no_w_alt,
            text_embeddings=alt_text_embeddings,
            guidance_scale=1,
            num_inference_steps=args.test_num_inference_steps,
        )
        
        fft_reversed_latent_no_w_alt = torch.fft.fftshift(torch.fft.fft2(reversed_latents_no_w_alt), dim=(-1, -2))[0][args.w_channel].real.detach().cpu().numpy()
        ...
        fft_reversed_latent_w = torch.fft.fftshift(torch.fft.fft2(reversed_latents_w), dim=(-1, -2))[0][args.w_channel].real.detach().cpu().numpy()
        
        image_latents_w_alt = altpipe.get_image_latents(img_w, sample=False)

        reversed_latents_w_alt = altpipe.forward_diffusion(
            latents=image_latents_w_alt,
            text_embeddings=alt_text_embeddings,
            guidance_scale=1,
            num_inference_steps=args.test_num_inference_steps,
        )
        
        fft_reversed_latent_w_alt = torch.fft.fftshift(torch.fft.fft2(reversed_latents_w_alt), dim=(-1, -2))[0][args.w_channel].real.detach().cpu().numpy()

        # eval
        no_w_metric, w_metric = eval_watermark(reversed_latents_no_w, reversed_latents_w, watermarking_mask, gt_patch, args)
        no_w_metric_alt, w_metric_alt = eval_watermark(reversed_latents_no_w_alt, reversed_latents_w_alt, watermarking_mask, gt_patch, args)

        if args.reference_model is not None:        # measures between vanilla and watermarked generated images
        ...
        results.append({
            ...
            "no_w_metric_alt": no_w_metric_alt, "w_metric_alt": w_metric_alt,
        })

        no_w_metrics.append(-no_w_metric)
        w_metrics.append(-w_metric)
        no_w_metrics_alt.append(-no_w_metric_alt)
        w_metrics_alt.append(-w_metric_alt)

        if args.with_tracking:
            table.add_data(wandb.Image(orig_image_no_w), w_no_sim, wandb.Image(orig_image_w), w_sim, current_prompt, no_w_metric, w_metric,
                           wandb.Image(fft_reversed_latent_no_w), wandb.Image(fft_reversed_latent_no_w_alt), wandb.Image(fft_reversed_latent_w), wandb.Image(fft_reversed_latent_w_alt))
    ...
    aux, acc, low = compute_eval(no_w_metrics, w_metrics)
    aux_alt, acc_alt, low_alt = compute_eval(no_w_metrics_alt, w_metrics_alt)
    ...
    if args.with_tracking:
        ...
        wandb.log({'clip_score_mean': mean(clip_scores), 'clip_score_std': stdev(clip_scores),
                   ...
                   'auc': aux, 'acc':acc, 'TPR@1%FPR': low, 'auc (alt)': aux_alt, 'acc (alt)':acc_alt, 'TPR@1%FPR (alt)': low_alt})
        wandb.finish()
    
    print(f'clip_score_mean: {mean(clip_scores)}')
    print(f'w_clip_score_mean: {mean(clip_scores_w)}')
    print(f'auc: {aux}, acc: {acc}, TPR@1%FPR: {low}')
    print(f'FOR ALT MODEL: auc: {aux_alt}, acc: {acc_alt}, TPR@1%FPR: {low_alt}')
    

def compute_eval(no_w_metrics, w_metrics):
